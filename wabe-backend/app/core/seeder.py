...
class RegionFactory:
    ...
    def build_region(self):
        ...
        for i in range(self.REGION_SIZE - 1):
            self._seed(region.terrain, i, 0, TERRAIN[terrain_type])
            terrain_type = self._grow(region.terrain, i, 0, terrain_type)
        return region
    
    def _seed(self, terrain, target_row, target_col, type):
        terrain[target_row][target_col] = type()
    
    def _grow(self, terrain, origin_row, origin_col, origin_type):
        #TODO algo to make biome boundaries organically
        #based on Terrain.solidity transient property
